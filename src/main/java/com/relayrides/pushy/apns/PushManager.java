...
public class PushManager<T extends ApnsPushNotification> implements ApnsConnectionListener<T> {
  ...
  public ExecutorService rejectedNotificationExecutorService;
 ...
 */
  public PushManager(  final ApnsEnvironment environment,  final SSLContext sslContext,  final int concurrentConnectionCount,  final NioEventLoopGroup eventLoopGroup,  final BlockingQueue<T> queue){
 ...
 */
  ...
  /** 
 * <p>Registers a listener for notifications rejected by APNs for specific reasons. Note that listeners are stored as strong references; all listeners are automatically un-registered when the push manager is shut down, but failing to unregister a listener manually or to shut down the push manager may cause a memory leak.</p>
