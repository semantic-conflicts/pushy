...
public class PushManager<T extends ApnsPushNotification> implements ApnsConnectionListener<T> {
  ...
  public LinkedBlockingQueue<T> retryQueue;
  public ApnsEnvironment environment;
  public SSLContext sslContext;
  public int concurrentConnectionCount;
  public ApnsConnectionPool<T> connectionPool;
  public FeedbackServiceClient feedbackServiceClient;
  public Vector<RejectedNotificationListener<? super T>> rejectedNotificationListeners;
  public Vector<FailedConnectionListener<? super T>> failedConnectionListeners;
  public Thread dispatchThread;
  public NioEventLoopGroup eventLoopGroup;
  public boolean shouldShutDownEventLoopGroup;
  public ReentrantLock connectionLock=new ReentrantLock();
  public Condition connectionsFinished=this.connectionLock.newCondition();
  public volatile int unfinishedConnectionCount=0;
  public ExecutorService listenerExecutorService;
  public boolean shouldShutDownListenerExecutorService;
  public boolean started=false;
  public boolean shutDown=false;
  public boolean shutDownFinished=false;
  public Logger log=LoggerFactory.getLogger(PushManager.class);
public static class DispatchThreadExceptionHandler<T extends ApnsPushNotification> implements UncaughtExceptionHandler {
    public Logger log=LoggerFactory.getLogger(DispatchThreadExceptionHandler.class);
    PushManager<T> manager;
 ...
 * @param eventLoopGroup the event loop group this push manager should use for its connections to the APNs gateway andfeedback service; if  {@code null}, a new event loop group will be created and will be shut down automatically when the push manager is shut down. If not   {@code null}, the caller <strong>must</strong> shut down the event loop group after shutting down the push manager.
 * @param listenerExecutorService the executor service this push manager should use to dispatch notifications toregistered listeners. If  {@code null}, a new single-thread executor service will be created and will be shut down automatically with the push manager is shut down. If not   {@code null}, the caller <strong>must</strong> shut down the executor service after shutting down the push manager.
 * @param queue the queue to be used to pass new notifications to this push manager
 */
  public PushManager(  final ApnsEnvironment environment,  final SSLContext sslContext,  final int concurrentConnectionCount,  final NioEventLoopGroup eventLoopGroup,  final ExecutorService listenerExecutorService,  final BlockingQueue<T> queue){
  ...
  /** 
 * <p>Opens all connections to APNs and prepares to send push notifications. Note that enqueued push notifications will <strong>not</strong> be sent until this method is called.</p> <p>Push managers may only be started once and cannot be reused after being shut down.</p>
 ...
 * @return {@code true} if this push manager has been started and has not yet been shut down or {@code false}otherwise
 */
 ...
 * @return {@code true} if this push manager has been shut down or is in the process of shutting down or{@code false} otherwise
 */
  public boolean isShutDown(){
    return this.shutDown;
  }
  /** 
 * Disconnects from APNs and gracefully shuts down all connections. This method will block until all connections have shut down gracefully.
 ...
 */
  ...
  /** 
 * Disconnects from the APNs and gracefully shuts down all connections. This method will wait until the given timeout expires for connections to close gracefully, and will then instruct them to shut down as soon as possible (and will block until shutdown is complete). Note that the returned list of undelivered push notifications may not be accurate in cases where the timeout elapsed before the client threads shut down.
 ...
 * @return {@code true} if the given listener was registered with this push manager and removed or {@code false} ifthe listener was not already registered with this push manager
 */
 ...
 */
  ...
  /** 
 * <p>Queries the APNs feedback service for expired tokens using a reasonable default timeout. Be warned that this is a <strong>destructive operation</strong>. According to Apple's documentation:</p> <blockquote>The feedback service’s list is cleared after you read it. Each time you connect to the feedback service, the information it returns lists only the failures that have happened since you last connected.</blockquote> <p>The push manager must be started before calling this method.</p>
 ...
 * @param timeout the time after the last received data after which the connection to the feedback service shouldbe closed
 ...
 */
  ...
  public void waitForAllOperationsToFinish(  final Date deadline) throws InterruptedException {
    this.connectionLock.lock();
    try {
